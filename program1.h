...
#define BloqueoUnicoMutEx(x)                unique_lock<std::mutex> x
#define BloqueoUnicoDiferidoMutEx(b,m)      unique_lock<std::mutex> b {m,defer_lock}
#define ResguardaMutEx(m)                   std::lock_guard<std::mutex> guard(m)
#define VariableDeCondicion                 std::condition_variable
#define esperaFuturo(f,t)                   f.wait_for(t)
#define EsperaCondicion(c)                  c.wait()
#define AvisaAUnoCondicion(c)               c.notify_one()
#define atomico(tipo)                       std::atomic<tipo>
#define futuro(tipo)                        std::future<tipo>
#define estadoFuturo                        std::future_status
